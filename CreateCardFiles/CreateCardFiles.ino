/*
   PopulateDB.ino
   Populate Flash Cards utility

   Thanks to https://github.com/firebull/arduino-edb/ for the SD CARD example. 

   The Extended Database library project page is here:
   http://www.arduino.cc/playground/Code/ExtendedDatabaseLibrary

*/

#include "Arduino.h"


// Use the SD card as storage. If you use external card, you will need SPI.h also 
#include <SD.h>

#define SD_PIN BUILTIN_SDCARD  // I used a Teensy 4.1 with integrated card

const int MAX_STRING_LEN = 256; // 255 characters and one for null character

#define STRINGS_TO_PROCESS 1990 // We load ### flash cards at a time into DB


File cardFile;



// Static array of string stored in flash to
// to be written to the database

// Ensure strings are stored in flash memory. I loaded 400 at a time, with the last batch
// being 390, for a total of 995 Flash Cards
static PROGMEM const char* stringvals[STRINGS_TO_PROCESS] = {
"abakada",
"ABC, alphabet, elements, rudiments",
"abenida",
"avenue",
"abo",
"ash, cinder",
"Abril",
"April",
"akayin",
"guide",
"akitin",
"attract",
"aklat",
"book",
"aklatan",
"library",
"akyatin",
"climb",
"agali",
"behavior, behaviour, conduct, deportment",
"agham",
"science",
"Agosto",
"August",
"agrikultura",
"agriculture, farming, tillage",
"agwat",
"distance, offset",
"ahas",
"serpent, snake",
"ahente",
"agent",
"air con",
"air conditioning",
"alak",
"wine",
"alahas",
"gem, jewel",
"alalahanin",
"recall, recollect, remember",
"alkohol",
"alcohol",
"ale",
"aunt",
"Alemanya",
"Germany",
"alikabok",
"dust",
"alila",
"boy, servant",
"alinlangan",
"doubt",
"aliw",
"consolation",
"aliwan",
"amusement, fun",
"aliwin",
"comfort, console",
"alon",
"wave",
"alpabeto",
"ABC, alphabet, elements, rudiments",
"altar",
"altar",
"alukin",
"offer, offer up, sacrifice",
"aluminyo",
"aluminium, aluminum",
"ama",
"father",
"ambisyon",
"ambition",
"ambulansiya",
"ambulance, field hospital",
"Amerika",
"America",
"amerikana",
"coat, overcoat",
"Amerikano",
"American, US citizen",
"ampunin",
"adopt",
"anak na babae",
"daughter",
"anak na lalaki",
"son",
"anghel",
"angel",
"ani",
"harvest",
"anim",
"six",
"animnapû",
"sixty",
"anluwagi",
"carpenter",
"anunsiyo",
"ad, advertisement",
"anyayahan",
"invite",
"apat",
"four",
"apatnapû",
"forty",
"apelyido",
"surname",
"aplaya",
"beach",
"apoy",
"fire, flame",
"araro",
"plough",
"araw",
"day",
"araw",
"sun",
"arkitekto",
"architect",
"arte",
"art",
"artipisyal",
"artificial",
"artista",
"actor, artist",
"asawa",
"husband",
"asin",
"salt",
"aso",
"dog",
"asukal",
"sugar",
"asul",
"blue",
"at",
"and",
"at saka",
"and",
"awa",
"compassion, pity",
"awit",
"song",
"awtomobil",
"automobile, car, motor car",
"ayon sa",
"according as, according to, as, by",
"baba",
"chin",
"babae",
"female, woman",
"baboy",
"hog, pig, swine",
"baka",
"calf",
"bakal",
"iron",
"bakasyon",
"vacation",
"bag",
"hand-bag, bag, sack",
"baga",
"lung",
"bagay",
"affair, business, business deal, case, matter",
"bagay",
"thing, article, object, thing",
"bago",
"before, in front of",
"bago",
"ahead, formerly, previously, new, novel",
"bahay",
"house",
"bala",
"bullet",
"balak",
"intention, meaning, plan",
"balahibo",
"feather",
"balat",
"skin",
"balbas",
"beard",
"baliw",
"crazy, insane, mad, nuts",
"balutan",
"packet, parcel",
"banda",
"direction",
"bandila",
"banner, flag",
"bangka",
"boat",
"bangko",
"bank",
"bansa",
"nation",
"bantayan",
"watch",
"bantayog",
"monument",
"bantog",
"popular",
"banyo",
"bathroom",
"bapor",
"steamship, ship, vessel",
"barbero",
"barber",
"baril",
"gun, rifle",
"baro",
"dress, gown, robe, shirt",
"basket",
"basket",
"baso",
"glass",
"bata",
"child, bairn, infant, young",
"batang babae",
"girl, lass, wench",
"batang lalaki",
"boy, lad, laddie",
"batas",
"law",
"bato",
"stone",
"bayad",
"payment",
"bayan",
"nation, city, town",
"bayani",
"hero",
"bayaw",
"brother-in-law",
"bentilador",
"electric fan, ventilator",
"berde",
"green",
"beynte",
"twenty",
"bibig",
"mouth",
"bigas",
"rice",
"bigat",
"weight",
"bigla",
"abrupt, sudden",
"bigyan",
"give",
"bilang",
"number, numeral",
"bilangguan",
"jail, gaol, prison",
"bilhin",
"buy, purchase, take over",
"bilibid",
"jail, gaol, prison",
"binata",
"bachelor",
"bintana",
"window",
"binti",
"leg, paw",
"biro",
"gag, joke",
"biruin",
"jest, joke",
"biskuwit",
"biscuit, cookie",
"bisikleta",
"bike, cycle, bicycle",
"bisig",
"arm",
"bistek",
"steak, beefsteak",
"bituin",
"star",
"biyenang babae",
"mother-in-law",
"biyenang lalaki",
"father-in-law",
"Biyernes",
"Friday",
"bola",
"ball",
"bolpoint",
"ballpoint pen",
"boses",
"voice",
"bota",
"boot",
"bote",
"bottle",
"boto",
"voice",
"braso",
"arm",
"bubungan",
"roof",
"bukas",
"open, tomorrow",
"bukas",
"open",
"bukas",
"tomorrow",
"bukid",
"estate, farm, property, ranch",
"buksan",
"open, open up",
"bughaw",
"blue",
"buhangin",
"sand",
"buhat sa",
"from, out of",
"buhay",
"alive",
"buhok",
"hair",
"buhol",
"knot, node",
"bulak",
"cotton",
"bulaklak",
"bloom, flower",
"bulati",
"worm",
"bulsa",
"pocket",
"bumasa",
"read",
"bumili",
"buy, purchase, take over",
"bundok",
"mountain",
"bungang-kahoy",
"fruit",
"bunhi",
"seed",
"buntot",
"tail",
"buslo",
"basket",
"butas",
"hole",
"buto",
"bone, seed",
"butones",
"button",
"buwan",
"month",
"buwis",
"tax",
"kaagad",
"abrupt, sudden",
"kaawaawa",
"charitable, merciful",
"kabayaran",
"payment",
"kabayo",
"horse",
"kabukiran",
"estate, farm, property, ranch",
"kabuuan",
"quantity",
"kagandahan",
"beauty",
"kagatin",
"bite",
"kahapon",
"yesterday",
"kahihiyan",
"shame",
"kahilingan",
"ask, ask for, beg, bid, request",
"kahi't",
"even, though, although",
"kahit na",
"though, although",
"kahon",
"box",
"kahoy",
"timber, wood",
"kaibigan",
"friend",
"kailangan",
"necessary",
"kalahati",
"half",
"kalawang",
"rust",
"kalayaan",
"freedom",
"kalikasan",
"character, nature",
"kalihim",
"secretary",
"kaliwa",
"left",
"kalusugan",
"health",
"kalye",
"street, road, route, way",
"kama",
"bed",
"kamalian",
"aberration, error, mistake",
"kamatayan",
"death",
"kamay",
"hand",
"kambing",
"goat",
"kamisadentro",
"shirt",
"kampana",
"bell",
"kanluran",
"west, West",
"kanta",
"song",
"kapatid na babae",
"sister",
"kapatid na lalaki",
"brother",
"kape",
"cafe, coffee-house",
"kapisanan",
"organization",
"kapitbahay",
"neighbour",
"karayom",
"needle",
"karbon",
"coal",
"karimlan",
"darkness, murk",
"karne",
"flesh",
"karne ng baka",
"beef",
"karpintero",
"carpenter",
"karunungan",
"wisdom",
"kasalanan",
"sin, transgression",
"kasama ng",
"among, between",
"kasangkapan",
"instrument",
"kasaysayan",
"story, history",
"kasiyahang-loob",
"fun, pleasure",
"kasunduan",
"accord, accordance, agreement, concurrence",
"katad",
"leather",
"katahimikan",
"peace",
"katalinuhan",
"wisdom",
"katapusan",
"end, ending",
"katas",
"juice, sap",
"katawan",
"body",
"katibayan",
"proof, sign, token",
"katitikan",
"ABC, alphabet, elements, rudiments",
"katorse",
"fourteen",
"katulong",
"aid, assistant, helper, boy, servant",
"katwiran",
"account, reason",
"kawani",
"employee",
"kawayan",
"bamboo",
"kaysa",
"than",
"kayumanggi",
"brown",
"keik",
"cake",
"keso",
"cheese",
"kilala",
"popular",
"kilay",
"brow, eyebrow",
"kinse",
"fifteen",
"kisame",
"ceiling",
"klase",
"class, property, quality",
"klase",
"class",
"klub",
"club, society",
"kopa",
"cup",
"kopya",
"copy",
"kopyahin",
"copy",
"kulay",
"colour, dye",
"kulay-abo",
"grey",
"kulisap",
"bug, insect",
"kulog",
"thunder",
"kumilos",
"actuate, move",
"kumot",
"blanket",
"kumpas",
"rhythm",
"kung",
"if, provided that",
"kunin",
"get, lay hold of, pick up, take",
"kurtina",
"curtain",
"kuru-kuro",
"idea, opinion",
"kurus",
"cross",
"kusinero",
"cook, male cook",
"kutsara",
"spoon",
"kutsilyo",
"knife",
"kuwarenta",
"forty",
"kuwatro",
"four",
"kuwelyo",
"collar",
"kuwenta",
"account",
"kuwero",
"leather",
"daan",
"street, road, route, way",
"dakila",
"big, great, large",
"Dakilang Britanya",
"Britain, Great Britain",
"dako",
"direction",
"daga",
"rat",
"dagat",
"sea",
"Dagat Itim",
"Black Sea, Euxine Sea",
"Dagat na Patay",
"Dead Sea",
"dahilan",
"cause, reason, account",
"dahil sa",
"because of, for, for sake of, on account of, owing to, through",
"dahon",
"leaf, sheet, page",
"dalandan",
"orange",
"dalaw",
"visit",
"dalawa",
"two",
"dalawampû",
"twenty",
"dalhin",
"bring, fetch, carry, wear",
"daliri",
"finger",
"dalisay",
"clean, pure",
"dambana",
"altar",
"damo",
"grass, herb",
"dati",
"ahead, formerly, previously",
"dibdib",
"chest",
"dila",
"tongue",
"dilaw",
"yellow",
"din",
"also, likewise, too",
"dingding",
"wall",
"dingmaan",
"war",
"di-pangkaraniwan",
"particular, special",
"disinuwebe",
"nineteen",
"disiotso",
"eighteen",
"disiseis",
"sixteen",
"disisiyete",
"seventeen",
"Disyembre",
"December",
"di-tapat",
"false",
"dito",
"here",
"Diyaba",
"Java",
"diyes",
"ten",
"dos",
"two",
"dose",
"twelve",
"dugo",
"blood",
"dula",
"drama",
"dumalaw",
"attend, call on, see, visit",
"dumating",
"arrive, end up, get",
"durungawan",
"window",
"edad",
"age",
"elektrika",
"electric",
"Eritrea",
"Eritrea",
"erograma",
"aerogramme, air letter",
"eroplano",
"aeroplane, airplane, plane, aircraft",
"Espanya",
"Spain",
"Estados Unidos",
"United States of America, USA",
"Europa",
"Europa, Europe",
"gabi",
"night, evening",
"galerya ng sining",
"art gallery",
"galit",
"angry, cross",
"galit",
"anger",
"gamitin",
"employ, use, make use of",
"gamot",
"drug, medicine, pharmaceutical",
"gana",
"appetite",
"ganda",
"beauty",
"garahe",
"garage",
"gas",
"gas",
"gatas",
"milk",
"gawain",
"job, work",
"gawi",
"behavior, behaviour, conduct, deportment",
"gawin",
"achieve, act, do, make, perform",
"ginto",
"gold",
"gising",
"awake",
"gitna",
"centre, average, mean, middle",
"Gitnang Amerika",
"Central America",
"Gitnang Europa",
"Central Europe",
"giyera",
"war",
"guhit",
"line",
"gulang",
"age",
"gulay",
"vegetable",
"gulong",
"wheel",
"gumalaw",
"actuate, move",
"guro",
"instructor, teacher",
"gusali",
"building",
"gusto",
"appreciate, like",
"guyam",
"ant, emmet",
"Haba",
"Java",
"hagdanan",
"staircase, stairs",
"halaga",
"quantity, price, value, worth",
"halalan",
"choice, election",
"halamanan",
"garden",
"halik",
"kiss",
"halimbawa",
"example",
"halos",
"about, approximately",
"halos",
"almost, nearly, at all, entirely, quite, wholly",
"hanapbuhay",
"job, work",
"handa na",
"already, by now, yet",
"hangarin",
"intention, meaning, plan",
"hanggang",
"until, till",
"hangin",
"air",
"hangin",
"wind",
"hapag",
"table",
"hapon",
"afternoon",
"Hapon",
"Japan, Japanese",
"hardin",
"garden",
"hatiin",
"divide, separate, share",
"hayop",
"animal, beast",
"heneral",
"general",
"hilaga",
"north, North",
"Hilagang Amerika",
"North America",
"hiling",
"ask, ask for, beg, bid, request",
"himpilan",
"station",
"hindi totoo",
"false",
"hininga",
"breath",
"hinlalaki",
"thumb",
"hinlalaki ng paa",
"toe",
"hintayin",
"abide, expect, wait, await, wait for",
"hipan",
"blow",
"hipuin",
"touch",
"hiwain",
"cut, slice",
"hiwalay",
"apart, particular, separate, special",
"hiya",
"shame",
"hubad",
"bare, naked, nude",
"hukbo",
"army",
"hukom",
"judge",
"huli",
"late, tardy",
"huli sa lahat",
"last",
"Hulyo",
"July",
"humiram",
"borrow",
"Hunyo",
"June",
"husto",
"exact, accurate, precise, enough, quite, rather, sufficiently",
"Huwebes",
"Thursday",
"huwes",
"judge",
"iba",
"other, another, else, different",
"ibig",
"appreciate, like",
"ibigay",
"give",
"ibigin",
"love",
"ibon",
"bird",
"iboto",
"vote",
"ikaanim",
"sixth",
"ikaapat",
"fourth",
"ikalawa",
"second",
"ikalima",
"fifth",
"ikapito",
"seventh",
"ikasampû",
"tenth",
"ikasiyam",
"ninth",
"ikatlo",
"third",
"ikawalo",
"eighth",
"igalang",
"respect",
"ihalal",
"vote",
"ihayag",
"publish, reveal",
"ilagay",
"lay down, place, put, put down",
"ilalim",
"bottom, foundation, ground",
"ilan",
"a few, any, some",
"ilarawan",
"describe",
"ilaw",
"light",
"ilog",
"river",
"Ilog Danubyo",
"Danube",
"ilong",
"nose",
"ina",
"mother",
"Inero",
"January",
"ingay",
"ado, din, noise",
"init",
"heat",
"insalada",
"salad",
"inumin",
"beverage, drink",
"ipadala",
"send, transmit",
"ipirito",
"fry",
"isa",
"one",
"isang libo",
"thousand, one thousand",
"isara",
"close, shut",
"iskuwela",
"school",
"isda",
"fish",
"isip",
"mind",
"istasyon",
"station",
"itago",
"hang onto, hold",
"itanong",
"ask",
"itim",
"black",
"itlog",
"egg",
"itulak",
"push, thrust",
"ituro",
"guide",
"iwasan",
"avoid, evade",
"iyak",
"cry, shout",
"labaha",
"razor",
"laban sa",
"across from, against, in exchange for, opposed to, opposite, upon",
"labi",
"lip",
"labimpito",
"seventeen",
"labindalawa",
"twelve",
"labing-anim",
"sixteen",
"labing-apat",
"fourteen",
"labing-isa",
"eleven",
"labingwalo",
"eighteen",
"labinlima",
"fifteen",
"labinsiyam",
"nineteen",
"labintatlo",
"thirteen",
"lakas",
"force, strength, vigour",
"laki",
"bulk, dimension, extend, size",
"lagda",
"signature",
"lagi",
"always, ever",
"lagnat",
"ague, fever",
"lahat",
"all, altogether, everything, general, usual",
"lalaki",
"male, masculine, fellow, man",
"lalamunan",
"throat",
"lalong mabuti",
"better",
"laman",
"flesh",
"lamang",
"exclusively, just, only, alone, solely",
"lamok",
"mosquito",
"langaw",
"fly, housefly",
"langgam",
"ant, emmet",
"langis",
"oil",
"langit",
"heaven, sky",
"lango",
"sober",
"lanseta",
"knife",
"lapagan ng eroplano",
"aerodrome, airdrome, airfield, airport",
"lapis",
"pencil",
"laro",
"game",
"lasing",
"sober",
"lason",
"poison",
"lata",
"tin",
"layo",
"distance, offset",
"leeg",
"neck",
"libangan",
"amusement, fun",
"libingan",
"grave, tomb",
"libro",
"book",
"likod",
"back",
"liham",
"letter",
"lihim",
"secret",
"lima",
"five",
"limampû",
"fifty",
"linggo",
"week",
"linggo",
"week",
"lipon",
"group",
"listahan",
"list",
"lubid",
"cord, rope, string",
"luha",
"tear",
"luma",
"old",
"lumaban",
"protest",
"lumakad",
"march, walk",
"lumangoy",
"float, swim",
"lumipad",
"fly",
"lumukso",
"jump, leap, spring",
"Lunes",
"Monday",
"lunsod",
"city, metropolis",
"luntian",
"green",
"lupa",
"country, earth, land, soil",
"lupon",
"committee",
"maaga",
"early",
"maalat",
"salty",
"maasim",
"acetous, acid, sour, tart",
"maawain",
"affable, friendly, good-natured, kind",
"mababa",
"low",
"mababang-loob",
"humble",
"mababaw",
"shallow, superficial",
"mabait",
"affable, friendly, good-natured, kind",
"mabalag",
"slow",
"mabilis",
"fast, quick, rapid, speedy, swift",
"mabuhay",
"be alive, live",
"mabuti",
"good, nice, okay",
"makalawa",
"twice",
"makapal",
"bold, corpulent, fat, thick",
"makina",
"engine, machine",
"makinig",
"listen",
"makita",
"see",
"makitid",
"narrow",
"madalang",
"leasurely, slowly",
"madali",
"easy, facile, fast, quick, rapid, speedy, swift",
"madilim",
"dark, dim",
"madulas",
"smooth",
"maestra",
"instructor, teacher",
"magaan",
"light",
"magaling",
"good, nice, okay",
"mag-anak",
"family",
"maganda",
"beautiful, fine, handsome, lovely",
"mag-aral",
"study",
"magbahing",
"sneeze",
"magbigay",
"give",
"magkaiba",
"different",
"magkaparis",
"equal, even, level",
"magkasama",
"together",
"magkatumbas",
"equal, even, level",
"magdiwang",
"celebrate",
"maghintay",
"abide, expect, wait, await, wait for",
"maglakbay",
"travel",
"maglalaro",
"play",
"magluto",
"cook",
"magnanakaw",
"thief",
"magpahinga",
"repose, rest",
"magpayo",
"advise, counsel",
"magsalita",
"speak, talk",
"magsasaka",
"farmer",
"magulang",
"parent",
"mahaba",
"long",
"mahalaga",
"important, serious",
"mahalin",
"love",
"mahina",
"faint, light, weak, slow",
"mahirap",
"miserable, poor",
"mahulog",
"drop, fall",
"mahusay",
"good, nice, okay",
"maikli",
"brief, short",
"mainit",
"warm, hot",
"maintindihan",
"understand, realize",
"maitim",
"black",
"malakas",
"strong, loud",
"malaki",
"big, great, large",
"malalim",
"deep, profound",
"malaman",
"know, know how",
"malambot",
"soft",
"malamig",
"bleak, chilly, cold",
"malapit",
"at, beside, by, near, nearby, near to, next to",
"malaya",
"free",
"malayo",
"away, distant, far, remote",
"mali",
"aberration, error, mistake",
"maligaya",
"fortunate, happy",
"maliit",
"diminutive, little, small",
"maliit na hayop",
"bug, insect",
"malinaw",
"clear, distinct, plain",
"malinis",
"clean, pure",
"maliwanag",
"clear, distinct, plain",
"malungkot",
"bleak, dismal, dreary, gaunt, miserable, sad, somber",
"malupit",
"cruel",
"malusog",
"healthy, well",
"mamon",
"cake",
"mananahi",
"tailor",
"manananggol",
"advocate, barrister, counsel, intercessor, lawyer, solicitor",
"manggagawa",
"hand, laborer, labourer, operative, worker, working man, workman",
"manika",
"doll, puppet",
"manok",
"chicken, fowl",
"mansanas",
"apple",
"mantekilya",
"butter",
"mapa",
"map",
"mapaghangad",
"ambitious",
"mapaglayon",
"ambitious",
"mapagpita",
"ambitious",
"mapait",
"acerbic, bitter",
"mapanganib",
"dangerous",
"marahil",
"maybe, mayhap, perchance, perhaps, possibly",
"Martes",
"Tuesday",
"martilyo",
"hammer",
"marumi",
"dirty, filthy, nasty, soiled, unclean",
"marunon",
"sagacious, sage, wise",
"masama",
"bad, miserable, nasty, poor",
"masaya",
"fortunate, happy",
"masipag",
"diligent, hardworking, industrious",
"masunurin",
"obedient",
"mata",
"eye",
"mataas",
"high, lofty, tall",
"matalas",
"abrasive, acerbic, acid, acrid, acrimonious, acute, lurid, sharp",
"matalino",
"sagacious, sage, wise",
"matamis",
"gentle, soft, sweet, tender",
"matanda",
"old",
"matapang",
"brave, courageous, valiant",
"matigas",
"hard",
"matinding galit",
"hate",
"matulin",
"fast, quick, rapid, speedy, swift",
"matulis",
"abrasive, acerbic, acid, acrid, acrimonious, acute, lurid, sharp",
"matulog",
"be asleep, sleep",
"maunawa",
"understand, realize",
"may",
"have, have got",
"mayaman",
"affluent, rich, wealthy, well-off",
"may-ari",
"owner",
"maybahay",
"wife",
"may gulang",
"adult, grown-up",
"Mayo",
"May",
"mayroon",
"have, have got",
"mesa",
"table",
"mil",
"thousand, one thousand",
"minuto",
"minute",
"miting",
"meeting",
"Miyerkules",
"Wednesday",
"mukha",
"face",
"mula sa",
"from, out of",
"muli",
"afresh, again, all over again, anew, once more",
"mundo",
"world",
"mura",
"cheap, inexpensive",
"na",
"already, by now, yet",
"nakalulugod",
"agreeable, enjoyable, lovely, nice, pleasant",
"nagiing",
"become, come about, grow, happen, get",
"nagtatanong",
"ask",
"naliligo",
"bathe, have a bath",
"nanay",
"mother",
"naniniwala",
"account, accredit, believe, deem",
"napaka-",
"quite, very, very much",
"natakot",
"afraid",
"negosyo",
"business, commerce, trade",
"ng",
"from, of",
"ngalan",
"appellation, name",
"ngayon",
"today, at present, now",
"ngipin",
"tooth",
"ngiti",
"smile",
"nguni't",
"but",
"Nilo",
"Nile",
"ningas",
"flame",
"ninong",
"godfather",
"ninuno",
"forefather",
"nobenta",
"ninety",
"noo",
"forehead",
"noong una",
"ahead, formerly, previously",
"nota",
"grade, note",
"Nubyembre",
"November",
"nuwebe",
"nine",
"Oktubre",
"October",
"onse",
"eleven",
"opera",
"opera",
"oras",
"hour, o'clock, time",
"orasan",
"clock, watch",
"orkesta",
"orchestra",
"ospital",
"hospital",
"otel",
"hotel",
"otsenta",
"eighty",
"otso",
"eight",
"paaralan",
"school",
"pako",
"nail",
"pakpak",
"wing",
"pag-aaral",
"education",
"pagamutan",
"hospital",
"pag-asa",
"hope",
"pagkakataon",
"chance, luck",
"pagkain",
"food, meal",
"pagkalipas",
"after, behind",
"pagdating",
"arrival",
"pagdiriwang",
"celebration",
"paggalang",
"respect",
"paghanga",
"admiration",
"pag-ibig",
"affection, love",
"paglalakbay",
"journey, trip, voyage",
"paglalarawan",
"account, description",
"paglusob",
"access, aggression, attack",
"pagod",
"tired",
"pagpayuhan",
"advise, counsel",
"pagsasaka",
"agriculture, farming, tillage",
"pahina",
"page",
"pahinga",
"repose, rest",
"pahintulot",
"accord, admit, afford, allow, permit",
"palakol",
"axe",
"palagay",
"idea, opinion",
"palagi",
"always, ever",
"palayaw",
"byname, nickname, sobriquet",
"palda",
"skirt",
"palengke",
"market, marketplace",
"palikuran",
"toilet",
"paliguan",
"bathroom",
"pamahalaan",
"administration, government",
"pambayan",
"public",
"pamilya",
"family",
"pamukpok",
"hammer",
"pana",
"arrow",
"panahon",
"time, while, weather",
"pananampalataya",
"religion",
"panga",
"jaw, jawbone",
"pangalan",
"appellation, name",
"pangalawa",
"second",
"pangamba",
"fear",
"pang-angkan",
"surname",
"panganib",
"danger, peril",
"pangkat",
"group",
"pangit",
"nasty, ugly",
"pangungusap",
"sentence",
"pantunaw",
"digestion",
"panulukan",
"angle, corner",
"papel",
"paper",
"paraan",
"process, system",
"pareho",
"same",
"paris",
"same",
"parisukat",
"square",
"paruparo",
"butterfly",
"pasaporte",
"passport",
"Pasko",
"Christmas, Yule",
"patatas",
"potato",
"patay",
"dead",
"paurong",
"backward, backwards",
"payat",
"gaunt, lean, slender, thin",
"payo",
"advice, counsel",
"payong",
"umbrella",
"Pebrero",
"February",
"pero",
"but",
"piitan",
"jail, gaol, prison",
"pilak",
"silver",
"pinakamabuti",
"best",
"pinggan",
"course, dish, plate, platter",
"pinsan",
"cousin",
"pintuan",
"door",
"pintura",
"dye, paint",
"Pirineo",
"Pyrenees",
"pirma",
"signature",
"pista",
"celebration",
"pito",
"seven",
"pitumpû",
"seventy",
"piyano",
"piano",
"pluma",
"pen",
"posporo",
"match",
"Pranses",
"Frenchman",
"Pransiya",
"France",
"propaganda",
"propaganda, publicity",
"pula",
"red",
"pulo",
"island",
"pulong",
"meeting",
"pulubi",
"beggar",
"pumarito",
"come",
"pumaroon",
"go",
"pumunta",
"go",
"puno",
"tree, boss, chief, leader, complete, full",
"puno",
"tree, boss, chief, leader",
"puno",
"complete, full",
"punong-kahoy",
"tree",
"pusa",
"cat",
"puso",
"heart",
"puti",
"blank, white",
"putulin",
"cut, slice",
"radyo",
"radio, wireless",
"relihiyon",
"religion",
"relo",
"clock, watch",
"restauran",
"restaurant",
"reyna",
"queen",
"riles",
"railroad, railway",
"rin",
"also, likewise, too",
"Roma",
"Rome",
"sa",
"from, out of, a, in, inside, into, on, per, within, at, beside, with",
"Sabado",
"Sabbath",
"sabaw",
"soup",
"sabihin",
"narrate, relate, tell",
"sabon",
"soap",
"sakit",
"ailment, disease, illness",
"sakit",
"ache, pain",
"sakit ng ulo",
"headache",
"sakuna",
"accident, catastrophe",
"saging",
"banana",
"sa gitna",
"among, between",
"sagot",
"answer, reply",
"sagutin",
"answer, reply, respond",
"sa harap ng",
"before, in front of",
"sa ibaba",
"below, downstairs, underneath",
"sa ibabaw",
"above, over, on top, overhead, upstairs, on, upon",
"sa ilalim",
"below, beneath, under, underneath",
"sa itaas",
"above, on top, overhead, upstairs",
"salakayin",
"assault, attack",
"salamin",
"glass",
"salapi",
"money",
"salawal",
"pants, trousers",
"sa likod ng",
"behind, after",
"salita",
"word",
"sa loob",
"a, in, inside, into, on, per, within",
"samahan",
"organization",
"sambahan",
"church, church-building, house of worship, place of worship",
"sambalilo",
"hat",
"sampû",
"ten",
"sandaan",
"hundred, one hundred",
"sandali",
"minute, time, while",
"sanggol",
"baby",
"sapagka't",
"as, because, for, since",
"sa paligid",
"about, round, around, towards",
"sa pamamagitan",
"by, by means of, on, through, with",
"sapat",
"enough, quite, rather, sufficiently",
"sapatos",
"shoe",
"sariwa",
"fresh, recent",
"sasakyan",
"vehicle",
"sastre",
"tailor",
"saya",
"skirt",
"seda",
"silk",
"seguro",
"insurance",
"seis",
"six",
"selyo",
"stamp, postage stamp",
"semento",
"cement",
"sepilyo",
"brush",
"serbesa",
"ale, beer",
"sesenta",
"sixty",
"setenta",
"seventy",
"Setyembre",
"September",
"siko",
"elbow",
"sigarilyo",
"cigarette",
"siguro",
"maybe, mayhap, perchance, perhaps, possibly",
"silangan",
"east, East",
"silanganan",
"eastern, Oriental",
"Silangang Indiyong Ulandes",
"Dutch East Indies",
"silid",
"chamber, room",
"silid-tulugan",
"bedroom",
"silya",
"chair",
"simbahan",
"church, church-building, house of worship, place of worship",
"siming",
"art",
"sinko",
"five",
"sinkuwenta",
"fifty",
"singaw",
"steam, vapor, vapour",
"singsing",
"ring",
"sinturon",
"belt, girdle",
"sinulid",
"thread",
"sipain",
"kick",
"sipi",
"copy",
"sipiin",
"copy",
"sira",
"damage",
"sirko",
"circus",
"sitaw",
"bean",
"siyam",
"nine",
"siyamnapû",
"ninety",
"siyensiya",
"science",
"siyento",
"hundred, one hundred",
"siyete",
"seven",
"sopas",
"soup",
"suki",
"client, customer",
"suklay",
"comb",
"sugat",
"injury, wound",
"sugo",
"ambassador",
"sulat",
"letter",
"sulok",
"angle, corner",
"sumagot",
"answer, reply, respond",
"sumalakay",
"assault, attack",
"sumalungat",
"protest",
"sumasangayon",
"accede, agree, consent",
"sumulat",
"write",
"sumunod",
"follow",
"sunog",
"fire",
"surbetes",
"ice, icecream",
"susi",
"key, wrench",
"tabako",
"tobacco",
"tabi",
"side",
"tabike",
"wall",
"takot",
"afraid",
"takot",
"fear",
"taksi",
"taxi",
"tagabili",
"client, customer",
"tagaluto",
"cook, male cook",
"tagapayo",
"counsellor",
"tag-araw",
"summer",
"tagiliran",
"side",
"taguan ng awto",
"garage",
"tagumpay",
"victory",
"tahanan",
"house",
"tahimik",
"calm, quiet",
"tala",
"star",
"talaan",
"list",
"tama",
"right, righthand, exact, accurate, precise",
"tama",
"exact, accurate, precise,",
"tama",
"right, righthand",
"tamad",
"lazy",
"tanggapin",
"accept, receive, take, take in, get, have, receive",
"tangi",
"particular, special",
"tanim",
"plant",
"tanong",
"question",
"tao",
"person",
"taon",
"year",
"tapang",
"courage",
"tapon",
"cork",
"tasa",
"cup",
"tatlo",
"three",
"tatlumpû",
"thirty",
"taunan",
"annual, yearly",
"tawa",
"laugh",
"taynga",
"ear",
"telebisyon",
"television, TV",
"telepono",
"telephone",
"tenis",
"tennis",
"ti'a",
"okay, well",
"tiket",
"bill, ticket, note",
"tignan",
"consider, deem, look, look at, regard, view, watch, see",
"tila",
"at all, entirely, quite, wholly",
"timog",
"south",
"Timog Amerika",
"South America",
"tinapay",
"bread, loaf",
"tingga",
"lead",
"tinig",
"voice",
"tinta",
"ink",
"tirahan",
"address",
"titik",
"letter",
"tiya",
"aunt",
"tiyan",
"stomach",
"totoo",
"true",
"tren",
"train",
"tres",
"three",
"trese",
"thirteen",
"treynta",
"thirty",
"tsuper",
"chauffeur, driver",
"tubig",
"water",
"tugon",
"answer, reply",
"tugtog",
"music",
"tugtugin",
"music",
"tugunin",
"answer, reply, respond",
"tuhod",
"knee",
"tulad",
"same",
"tulay",
"bridge",
"tulong",
"aid, help",
"tumakbo",
"run",
"tumalon",
"jump, leap, spring",
"tumawa",
"laugh",
"tumugon",
"answer, reply, respond",
"tumulong",
"abet, aid, assist, help",
"tunod",
"arrow",
"tupa",
"sheep",
"tuyo",
"dry",
"ubo",
"cough",
"ugali",
"custom, habit, way",
"ugat",
"root",
"ulan",
"rain",
"ulap",
"cloud",
"uli",
"afresh, again, all over again, anew, once more",
"uling",
"coal",
"ulo",
"head, pate",
"umaakyat",
"ascend, climb, go up",
"umakit",
"attract",
"umakyat",
"climb",
"umaga",
"morning",
"uminom",
"drink",
"umiyak",
"cry out, scream, shout",
"una",
"first, firstly, first of all",
"unggoy",
"ape, monkey",
"uno",
"one",
"uod",
"worm",
"upuan",
"chair",
"uri",
"property, quality, kind, sort",
"uri",
"property, quality, kind, sort",
"uri",
"class",
"usbong ng kawayan",
"bamboo shoots",
"usok",
"smoke",
"utak",
"brain",
"utang",
"debt",
"wakas",
"end, ending",
"wagas",
"clean, pure",
"walang-kibo",
"calm, quiet",
"walang-hiya",
"abandoned",
"walang pasok",
"vacation",
"walo",
"eight",
"walumpû",
"eighty",
"wasto",
"right, righthand",
"watawat",
"banner, flag",
"wika",
"language, tongue",
"wikain",
"dialect",
"wiski",
"whisky, whiskey",
"yelo",
"ice"
};




// Run the demo
void setup()
{

    Serial.begin(9600);

    if (!SD.begin(SD_PIN)) {
        Serial.println("No SD-card.");
        return;
    }
    createRecords();
}
void createRecords() 
{
  int curFile = 0;
  String thisFile;
  char fileName[8]; // 001.txt to 995.txt
  int n = 0;
  while(n < STRINGS_TO_PROCESS) {
    curFile++;
    sprintf(fileName,"%03d.txt",curFile);
    cardFile = SD.open(fileName, FILE_WRITE);
    // Write the two strings
    cardFile.println(stringvals[n++]);
    cardFile.println(stringvals[n++]);
    cardFile.close();
    Serial.print("Card #");
    Serial.print(curFile);
    Serial.println(" created.");
  }

  Serial.println("Cards Created");
}

// Nothing to do here, program is just to load strings into DB
void loop()
{
}
